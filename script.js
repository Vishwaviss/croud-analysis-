...
function setupEventListeners() {
    ...
    if (resetAllBtn) {
        resetAllBtn.addEventListener('click', () => {
            if (confirm("⚠️ CAUTION: This will delete ALL face data, all saved persons, and reset every counter. Are you sure?")) {
                sendToBackend({ action: 'reset_all' });
                // Clear local UI elements immediately
                const facesList = document.getElementById('facesList');
                if (facesList) facesList.innerHTML = '<p class="empty-state">System Resetting...</p>';
                const alertsList = document.getElementById('alertsList');
                if (alertsList) alertsList.innerHTML = '<p class="empty-state">System Resetting...</p>';
                document.getElementById('totalCount').textContent = '0';
                document.getElementById('peopleCount').textContent = '0';
                setTimeout(() => location.reload(), 1500); // Reload to clear tracking state
            }
        });
    }
}
